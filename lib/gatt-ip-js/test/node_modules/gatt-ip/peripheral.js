...
function Peripheral(gattip, uuid, name, rssi, addata, scanData) {
    ...
    this._updateFromScanData = function (name, rssi, txPwr, serviceUUIDs, mfrData, serviceData, addata, scanData) {
        this.name = name;
        this.rssi = rssi;
        ...
        if (serviceData) {
            for (var serUUID in serviceData) {
                manufacturerData[serUUID] = serviceData[serUUID];
            }
        }
        if (serviceUUIDs) {
            for (var sidx = 0; sidx < serviceUUIDs.length; sidx++) {
                pushUnique(this.serviceUUIDs, serviceUUIDs[sidx]);
            }
        }
        if (addata) {
            advDataParser.parseAdvArray(self, addata);
        }
